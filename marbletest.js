...
import { runCommand, getScore, tellraw, subtitle, actionbarRaw, getPlayerWithTag } from "./function.js";
import { Location, system, world } from "@minecraft/server";
const prefix = "marble";
let dicetick = 0;
let sneak;
let turnnum;
...
world.events.tick.subscribe(({ currentTick }) => {
    ...
    if (currentTick % 10 == 0) {
        ...
        [...world.getPlayers()].forEach((pl)=>{
            let kan = [...world.getDimension('overworld').getEntities({location:new Location(pl.location.x,pl.location.y-4,pl.location.z),closest:1,maxDistance:2})][0]
            if(kan) {
                let b = parseInt(kan.nameTag.slice(9))
                if (nationprice[b]==0)
        ...
        for (let k = 0; k < getScore(`players`, `marble`); k++) {
            ...
            let mp = [...world.getPlayers({tags:[`marble${k + 1}P`]})][0].nameTag;
...
world.events.tick.subscribe(({ currentTick }) => {
    ...
    let turnplLoc;
    if(turnpl) {
        turnplLoc = { x: Math.round(turnpl.location.x), y: Math.round(turnpl.location.y), z: Math.round(turnpl.location.z) }
    }
...
export function allCommand(data) {
    if (data.message.trim() == prefix + " start") {
        ...
        system.runSchedule(()=>{
            ...
            let getmarbleplayers = getPlayerWithTag(`marble${j}P`)
            if (getmarbleplayers) {
