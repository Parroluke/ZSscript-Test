import { world,EntityQueryOptions,BlockLocation, EntityHealthComponent } from "@minecraft/server";
import { runCommand } from "../util/function.js"
let broke: BlockLocation[] = [];
...
world.events.entityHurt.subscribe((data=>{
    ...
    const health = <EntityHealthComponent>data.hurtEntity.getComponent('health');
    if(health.current <= 0) broke.push(new BlockLocation(Math.floor(x),Math.floor(y),Math.floor(z)))
}))