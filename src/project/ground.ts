...
world.events.itemUse.subscribe((data)=>{
    ...
    if(data.item.typeId=="minecraft:emerald") {
        ...
        form.show(data.source as Player).then((r)=>{
            ...
            if(buttons[r.selection]=="땅 확장") {
                ...
                extendForm.show(source as Player)
                .then(r=>{
                    ...
                    if(!r.canceled) saveInfo(groundInfo)
                })
            }
            ...
            if(buttons[r.selection]=="땅 관리") {
                ...
                const players = [...world.getPlayers()].map((v: any) => v = v.nameTag)
                playerForm.title('권한 설정')
                playerForm.dropdown("플레이어",players,0)
                playerForm.show(source as Player)
                .then((r)=>{
                    ...
                    settingForm.show(source as Player)
                    .then(r=>{
                        const permi = new permission(source as Player)
                        if(r.formValues[0]) permi.setPermission("place",nameTag,true)
                        else permi.setPermission("place",nameTag,false)
                        if(r.formValues[1]) permi.setPermission("break",nameTag,true)
                        else permi.setPermission("break",nameTag,false)
                    })
                })
            }
...
function saveInfo(value: object) {
...
function includeBlock(loc: BlockLocation | Location): string | boolean {
    let groundInfo = getInfo()
    let r: string | boolean = false;
    Object.keys(groundInfo).forEach((gr)=>{   
        if(groundInfo[gr]!=null) {
            const {loc1,loc2} = groundInfo[gr]
            if(loc.x>=loc1[0]&&loc.x<=loc2[0]&&loc.y>=loc1[1]&&loc.y<=loc2[1]&&loc.z>=loc1[2]&&loc.z<=loc2[2]) r = gr
        }
    })
    return r;
}
...
class permission {
     ...
     */
    testBreak(gr: ground) {
        if(gr?.break.includes(this.plr.nameTag)) return true;
        else return false;
    }
    /**
     ...
     */
     testPlace(gr: ground) {
        if(gr?.place.includes(this.plr.nameTag)) return true;
        else return false;
    }
     ...
     */
    setPermission(permission: string, grSt: string, value: boolean) {
        let info = getInfo()
        let gr = info[grSt]
        let permissions = gr[permission].filter((v:string | Player) => v!=this.plr.nameTag); 
        if(value) permissions.push(this.plr.nameTag)
