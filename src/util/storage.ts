import { Block, BlockLocation, DynamicPropertiesDefinition, Entity, MinecraftEntityTypes, world, Player } from "@minecraft/server";
...
class worldStorage {
    ...
    constructor() {
        ...
        event.addEvent('playerJoin', (d)=>{
            if(!d.player.getTags().find(v=>v.startsWith("{"))) d.player.addTag("{}")
        })
    }
