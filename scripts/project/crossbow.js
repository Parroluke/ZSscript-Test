import { MinecraftEffectTypes, Vector, world } from "@minecraft/server";
import { runCommand, actionbarRaw } from "../util/function.js";
...
world.events.tick.subscribe(() => {
    [...world.getPlayers()].forEach(plr => {
        if (hasItem(plr, 'slot.armor.head', 0, 'netherite_helmet', -1))
            plr.addEffect(MinecraftEffectTypes.slowness, 10, 8, false);
    });
});
...
world.events.projectileHit.subscribe((data) => {
    if (data.entityHit)
        data.source.runCommand(`playsound random.orb @s ~~~`);
});
function hasItem(plr, location, slot, item, data) {
    try {
        let command = plr.runCommand(`testfor @s[hasitem={location=${location},slot=${slot},item=${item},data=${data}}]`).statusMessage;
        if (data == -1)
            command = plr.runCommand(`testfor @s[hasitem={location=${location},slot=${slot},item=${item}}]`).statusMessage;
    }
    catch (e) {
        return false;
    }
...
function arrow(plr, location, slot, item) {
    for (let a = 0; a < 44; a++) {
        if (hasItem(plr, location, slot, item, a))
            return a;
    }
    return false;
}
